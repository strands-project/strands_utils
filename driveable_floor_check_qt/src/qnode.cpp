...
#include <stdio.h>
#include <math.h>
#include "../include/DriveableFloorCheckQt/qnode.hpp"
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
namespace TopKinect {
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// Init the ROS stuff and create subscribers and a publisher.
    ros::init(init_argc, init_argv, "DriveableFloorCheckQt");
    ros::NodeHandle n("~");
    std::string cam_info_topic, disparity_img_topic, active_topic;
    n.getParam ( "cam_info_topic",  cam_info_topic);
    n.getParam ( "disparity_img_topic",  disparity_img_topic);
    n.getParam ( "active_topic",  active_topic);

    if( cam_info_topic.length() == 0 )
    {
        cam_info_topic = "/camera/depth_registered/camera_info";
    }
    if( disparity_img_topic.length() == 0 )
    {
        disparity_img_topic = "/camera/depth_registered/disparity";
    }
    if( active_topic.length() == 0 )
    {
        active_topic = "/camera/active";
    }

    std::cout << "Subscribed to camera info topic: " << cam_info_topic << std::endl
              << "Subscribed to disparity image topic: " << disparity_img_topic << std::endl
              << "Subscribed to active topic: " << active_topic << std::endl;

    ros::Subscriber sub1 = n.subscribe<sensor_msgs::CameraInfo>(cam_info_topic, 1, &QNode::cameraInfoCallback, this);
    ros::Subscriber sub2 = n.subscribe<stereo_msgs::DisparityImage>(disparity_img_topic, 1, &QNode::disparityCallback, this);

    std::cout << "active " << active << std::endl;
    ros::Subscriber activeSubs = n.subscribe<std_msgs::String>(active_topic,1,&QNode::activeCallback,this);
    std::cout << "active " << active << std::endl;
